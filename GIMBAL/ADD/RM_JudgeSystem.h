...
#define       LED_YELLOWMeaning                 "HAHAA"
#define       LED_GREENMeaning                  "Stupid"
#define       LED_ORANGEMeaning                 "Holyy"
#define       LED_PURPLEMeaning                 "Shitt"
#define       LED_PINKMeaning                   "I am "
#define       LED_CYANMeaning                   "Huang"
#define       LED_BLACKMeaning                  "jian "
#define       LED_WHITEMeaning                  "xian "
#define       LED_Init                          "          "
...
typedef struct 
{
	...
	//更新成功标志位
	uint8_t DataUpdate_Flag;
	//非裁判系统作者的变量
	WorldTime_RxTypedef WorldTimes;				//帧率的结构体变量
	uint32_t FPS;
	uint8_t Offline_Flag;						//离线标志位
	uint32_t Offline_Detec;						//离线检测
...
/*发送给哨兵(仅限一台步兵调用)*/
void Judge_SendInfantry(uint16_t);
/*发送给飞镖发射架(仅限空中机器人调用)*/
